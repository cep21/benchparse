...
ifndef REPO
	...
	git grep -l 'cep21/benchparse' | xargs perl -i -pe"s#cep21/benchparse#$(OWNER)/$(REPO)#g"
	git grep -l 'benchparse' | xargs perl -i -pe"s#benchparse#$(REPO)#g"
	mv benchparse.go $(REPO).go
	mv benchparse_example_test.go $(REPO)_example_test.go
	mv benchparse_test.go $(REPO)_test.go
	go test ./...
